...
public class SlabfishModel<E extends SlabfishEntity> extends EndimatorEntityModel<E> {
    ...
    public void setRotationAngles(E entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
    	...
    	if(!entityIn.isInWater()) {
    		...
    		this.rightLeg.rotateAngleX = (entityIn.isSitting() || entityIn.getRidingEntity() != null) ? -1.57F :  MathHelper.cos(limbSwing * 0.6662F) * 1.5F * limbSwingAmount;
            this.leftLeg.rotateAngleX = (entityIn.isSitting() || entityIn.getRidingEntity() != null) ? -1.57F :  MathHelper.cos(limbSwing * 0.6662F + (float)Math.PI) * 1.5F * limbSwingAmount;
